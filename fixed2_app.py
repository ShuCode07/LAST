...
def load_data():
    ...
    try:
        ...
        # 为股票代码和年份添加索引，加速查询
        if '股票代码' in df.columns:
            df.set_index(['股票代码', '年份'], inplace=True)
...
if df is not None:
    ...
    # 获取企业名称映射 - 使用向量化操作代替循环
    stock_name_map = {}
    if '企业名称' in df.columns:
        # 使用drop_duplicates和set_index创建映射，比循环更快
        name_mapping_df = df[['股票代码', '企业名称']].dropna().drop_duplicates(subset=['股票代码'])
        stock_name_map = dict(zip(name_mapping_df['股票代码'], name_mapping_df['企业名称']))
    ...
    # 年份选择 - 优化查询速度
    # 使用布尔索引快速过滤
    stock_years = sorted(df.loc[df['股票代码'] == selected_stock, '年份'].unique())
    ...
    if selected_year == '全部显示':
        # 使用loc访问器加速查询
        selected_data = df.loc[df['股票代码'] == selected_stock]
    else:
        # 复合条件查询优化
        selected_data = df.loc[(df['股票代码'] == selected_stock) & (df['年份'] == selected_year)]
    ...
    if not selected_data.empty:
        ...
        if index_cols:
            # 只取第一行数据
            company_info = selected_data[index_cols].head(1).iloc[0]
        ...
        if len(selected_data) > 1 and index_cols:
            ...
            # 限制同时显示的指标数量，避免图表过于复杂
            max_series = 5
            plot_cols = index_cols[:max_series]
